...
5. **Cloudflare cache** — cache PAB price responses at the Cloudflare edge to reduce origin load. Further along than "not started": `Cache-Control: public, s-maxage=3600, stale-while-revalidate=60` is already set on all three PAB price endpoints (`/pab/price/{part_no}/{color_id}`, `/pab/prices/{part_no}`, `/element/{element_id}/price`), and the purge side is now correctly designed — `scripts/scrape_pab.py` purges only the specific URLs for elements whose price/stock actually changed (see PAB Scraper section above), instead of the old blanket `purge_everything` on every run. **But live testing 2026-07-16 found `cf-cache-status: MISS` on a real request, and the response's `Cache-Control` header arrives at the client with `, no-store` appended** — confirmed the raw origin (uvicorn, bypassing both the Cloudflare Tunnel and Caddy) returns the clean header with no `no-store`, so something between the tunnel and Cloudflare's edge is adding it, most likely a zone-level Cache setting (e.g. Cache Rules not marking these paths "Eligible for Cache," since Cloudflare doesn't cache dynamic API responses by default regardless of origin headers). **Not yet diagnosed or fixed** — needs a check of the Cloudflare dashboard's Cache Rules for the `api.moc-source.com` zone before this feature can actually deliver any edge-caching benefit; the purge logic is ready and waiting for it
