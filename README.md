...
| `fix_rebrickable_part_nos.py` | One-time correction pass for `bricklink_mappings.part_no` rows written with Rebrickable's own numbering instead of a real BrickLink part number (root-caused 2026-07-14) — two-tier resolution (bulk lookup + per-`element_id` fallback), tracked via `rebrickable_part_no_fixes`. 221 rows still unresolved after both tiers — pending manual review, see `~/projects/unresolved_rebrickable_parts.csv` (outside git, cross-referenced with year data from `backfill_last_used_year.py`) | Manual, resumable |
| `apply_manual_overrides.py` | Applies hand-verified corrections/dead-ends from `cache/manual_overrides.yaml` for rows `fix_rebrickable_part_nos.py` couldn't resolve automatically. Built and verified 2026-07-14, **not yet run** — waiting on the manual review pass above | Manual, as needed |
