...
| `bl_price_guide_velocity` | "Quick movers" signal (2026-07-16) — one rolled-up average-monthly-units-sold figure per (part_no, color_id, region, new_or_used), not a monthly time series. Computed by `scrape_bl_price_guide.py` in-memory from the same `price_detail[]` quantities it already fetches (summed from the outlier-filtered rows, so the same bogus bulk "sales" the price filter rejects don't inflate volume either), no new BL calls |
