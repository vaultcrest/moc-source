...
| `bl_part_catalog` | Cache for BL catalog API lookups (part_no PK, name, item_type, looked_up_at); also the mold-succession backfill's data: `last_used_year`, `year_released`, `supersets_count`, `mold_backfilled_at` (dedicated resumability gate, separate from `looked_up_at` — see What's Next #4) |
...
4. **Discontinued part / newer mold detection** — many parts (jumper plates especially) get re-tooled over the years into a new mold under the same base part number with a different letter suffix (e.g. `3684a` "hollow studs" 1978 vs `3684c` "solid studs" 2009). Idea: detect when a part in a user's list is an old, likely-discontinued mold and suggest the newer one, which may currently be on PAB.
   - **Data collection is built and running** (`scripts/scrape_bl_mold_data.py`, nightly systemd timer, batch of 2,500 `part_no`s/night ≈ 5,000 BL calls, resumable via the dedicated `bl_part_catalog.mold_backfilled_at` gate). Backfills `last_used_year`, `year_released`, `supersets_count` (added 2026-07-13 — distinguishes "BL confirms never in a set" from "BL found sets but none matched our local `lego_sets` snapshot," previously indistinguishable and a real data gap), and refreshes `bricklink_alternates`. **What's still undone is everything below this line** — the actual detection/suggestion logic and its surfacing in the API/UI.
...
8. **Firefox support** — planned, not started. Audit (2026-07-12) found the extension is already close to Firefox-compatible: every `chrome.storage`/`chrome.runtime`/`chrome.tabs` call already uses `await` promise style (not Chrome's old callback style), and no Chrome-only APIs are in use. Decided approach: **one shared master source tree, not two maintained copies** — only the manifest actually needs to differ per browser (`background.service_worker` vs `background.scripts`, `browser_specific_settings.gecko.id`, `strict_min_version` ≥128 for the `"world": "MAIN"` content-script key used by `content_main.js`). Plan (revised 2026-07-13 — symmetric layout, not "Chrome loads from root"): move the real files into `extension/master/` (all shared `.js`/`.html`/icons, plus both `manifest.chrome.json` and `manifest.firefox.json`), then add `extension/chrome/` and `extension/firefox/` as sibling folders containing **only symlinks** back into `master/` — each including a `manifest.json` symlink pointing at its own manifest variant. Both live on the Samba share as independent load-unpacked targets (`extension/chrome/` in Brave/Chrome, `extension/firefox/` via Firefox's `about:debugging` → Load Temporary Add-on or `web-ext run`); editing any file in `master/` updates both instantly, zero build step or bundler. Default `zip` behavior dereferences symlinks into real file content, so packaging a release zip from either `chrome/` or `firefox/` needs no special flags. Remaining unknowns: live-test the DOM logic (row injection, focus/blur simulation, MAIN-world `confirm()` override) actually against Firefox's renderer — nothing here can be verified by code review alone; then AMO submission (self-distribution signing is fast/automated if not doing a public listing yet). Aside noticed during the audit, unrelated to Firefox: `content-lego-spy.js` isn't referenced in `manifest.json`'s `content_scripts` nor injected anywhere found in `content-lego.js` — looks like dead code, worth checking separately.
