...
| `bl_part_catalog` | Cache for BL catalog API lookups (part_no PK, name, item_type, looked_up_at); also the mold-succession backfill's data: `last_used_year`, `year_released`, `mold_backfilled_at` (dedicated resumability gate, separate from `looked_up_at` — see What's Next #4) |
| `bricklink_alternates` | Alternate part numbers (e.g. x224 ↔ 41751 ↔ 40995); used by Rebrickable enrichment fallback |
| `bl_price_guide_monthly` | BrickLink Price Guide history, North America, sold — PK (part_no, color_id, region, new_or_used, month); avg/min/max/median in cents plus `sample_count` (post-outlier-filter) and `raw_sample_count`. One row per month bucket found in a scrape's `price_detail[]` (outlier-filtered per bucket — MAD/PAB-ratio/max-drop three-tier strategy, `scripts/_outlier_filter.py`). No retention limit — rows accumulate indefinitely for multi-year price-trend history (see What's Next #3) |
| `bl_price_guide_scan_log` | Thin resumability tracker for the price-guide first pass — PK (part_no, color_id, region), `scanned_at`. Decoupled from `bl_price_guide_monthly` so a pair with zero BL sales still gets marked done instead of retried forever |
...
| `scrape_bl_mold_data.py` | Backfills `bl_part_catalog.name`/`item_type`/`alternate_no` (mold-family siblings, refreshes `bricklink_alternates`) per part_no in `bricklink_mappings`, 1 BL call/part. Resumable via the dedicated `mold_backfilled_at` gate. **Throttled 2026-07-15** from 5,000 to 500 calls/night to free up budget for `scrape_bl_price_guide.py` below, since its output only feeds the not-yet-built discontinished-mold feature (What's Next #4) — nothing live consumes it yet | Nightly via systemd timer, 02:00 (500 calls/night) |
| `scrape_bl_price_guide.py` | First pass: BrickLink Price Guide (North America, sold, New+Used) into `bl_price_guide_monthly`, priority-ordered — PAB/BAP-priced pairs first, then by `bl_part_catalog.last_used_year` descending, then no-year-data pairs last. Each call returns up to ~6 months of real sold-listing history in one shot, bucketed by month and outlier-filtered (`scripts/_outlier_filter.py`) before being written. Resumable via `bl_price_guide_scan_log`. 2 BL calls/pair. `filter_bucket()` drops non-positive prices before any tier logic (BL's `price_detail[]` includes some genuine $0.0000 line items — fixed 2026-07-15 after two crashes on real data); any other per-bucket failure is caught, skips just that bucket, and is named (`part_no/color_id/new_or_used/month`) in the email report instead of killing the run | Nightly via systemd timer, 03:00 (2,250 pairs = ~4,500 calls/night) |
...
3. **BrickLink average/median price** — **first-pass scrape built and running as of 2026-07-15.** `bl_price_guide_monthly` (North America, sold, New+Used, monthly-bucketed — up to ~6 months of history backfilled per pair in one scrape) is populated by `scripts/scrape_bl_price_guide.py` on a nightly systemd timer (03:00, 2,250 pairs/night), same never-a-live-call-in-the-request-handler pattern as the Rebrickable scrape-time enrichment. Outlier rejection (three-tier MAD/PAB-ratio/max-drop, `scripts/_outlier_filter.py`) is applied per month-bucket before avg/min/max/median are written. No retention limit — history is kept indefinitely for multi-year price-trend charts, not just a current snapshot.
   ...
   - **Still to do**: join `bl_price_guide_monthly` (latest month) into the existing `/pab/price/{part_no}/{color_id}` response as `bl_avg_price_cents`/`bl_avg_price_formatted` — the extension's client side has been ready for this field since v0.4.11/12. An ongoing refresh cadence for already-scraped pairs (re-checking the current still-open month, periodically re-verifying older ones) is also undecided — this first pass only sweeps each pair once. Region is North America only for now; EU etc. would need new scrape calls, schema already supports it (`region` is part of the PK, not hardcoded)
4. **Discontinued part / newer mold detection** — many parts (jumper plates especially) get re-tooled over the years into a new mold under the same base part number with a different letter suffix (e.g. `3684a` "hollow studs" 1978 vs `3684c` "solid studs" 2009). Idea: detect when a part in a user's list is an old, likely-discontinued mold and suggest the newer one, which may currently be on PAB.
   - **Data collection is built and running.** `scripts/scrape_bl_mold_data.py` (nightly systemd timer, batch of 5,000 `part_no`s/night ≈ 5,000 BL calls — doubled from 2,500 on 2026-07-14 once the job dropped to 1 BL call/part, to keep using the full daily budget; ~38,846 part_nos remaining ≈ 7.8 more nights, resumable via the dedicated `bl_part_catalog.mold_backfilled_at` gate) backfills `name`/`item_type`/`alternate_no` and refreshes `bricklink_alternates`. `scripts/backfill_last_used_year.py` (monthly, full pass, ~25 min live) separately backfills `last_used_year`/`year_released` from Rebrickable's bulk parts API instead of a paced BrickLink call per part (2026-07-14 — faster and no longer dependent on `lego_sets` coverage, which previously left a gap for series like Collectible Minifigures). First run resolved 41,404/41,473. 67 part_nos (76 elements) still lack `last_used_year` — confirmed all 67 got zero year data from Rebrickable (not a resolution bug); of those, 43 already have a correct BL id (just obscure Rebrickable print variants Rebrickable itself has thin metadata for), 23 were never touched by the pr/pb bug, and exactly 1 (`100588pat0001pr0002`) overlaps with the 221-row unresolved-BL-id list above. **TODO**: once this set is triaged further, add a small nightly job retrying just it instead of waiting for the next monthly full pass. **What's still undone is everything below this line** — the actual detection/suggestion logic and its surfacing in the API/UI.
