...
- [x] **Cart Jigsaw pricing-accuracy overhaul (2026-07-17/18, not yet version-tagged — batched under CHANGELOG's `[Unreleased]` per the current release convention)**: a chain of real bugs found by working backward from a reported Project-vs-Cart total mismatch, each root-caused against live BrickLink data before being fixed:
  ...
  - See `CHANGELOG.md`'s `[Unreleased]` section for full technical detail on each fix
