...
4. **Discontinued part / newer mold detection** — many parts (jumper plates especially) get re-tooled over the years into a new mold under the same base part number with a different letter suffix (e.g. `3684a` "hollow studs" 1978 vs `3684c` "solid studs" 2009). Idea: detect when a part in a user's list is an old, likely-discontinued mold and suggest the newer one, which may currently be on PAB.
   - **Data collection is built and running.** `scripts/scrape_bl_mold_data.py` (nightly systemd timer, batch of 2,500 `part_no`s/night ≈ 2,500 BL calls, resumable via the dedicated `bl_part_catalog.mold_backfilled_at` gate) backfills `name`/`item_type`/`alternate_no` and refreshes `bricklink_alternates`. `scripts/backfill_last_used_year.py` (nightly, full pass, ~8 min) separately backfills `last_used_year`/`year_released` from Rebrickable's bulk parts API instead of a paced BrickLink call per part (2026-07-14 — faster and no longer dependent on `lego_sets` coverage, which previously left a gap for series like Collectible Minifigures). **What's still undone is everything below this line** — the actual detection/suggestion logic and its surfacing in the API/UI.
