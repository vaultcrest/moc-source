...
8. **Firefox support** — planned, not started. Audit (2026-07-12) found the extension is already close to Firefox-compatible: every `chrome.storage`/`chrome.runtime`/`chrome.tabs` call already uses `await` promise style (not Chrome's old callback style), and no Chrome-only APIs are in use. Decided approach: **one shared source tree, not two maintained folders** — only `manifest.json` actually needs to differ per browser (`background.service_worker` vs `background.scripts`, `browser_specific_settings.gecko.id`, `strict_min_version` ≥128 for the `"world": "MAIN"` content-script key used by `content_main.js`). Plan: keep `extension/manifest.json` as Chrome's real manifest (dev workflow unchanged), add `extension/manifest.firefox.json`, and add `extension/dist/firefox/` as a folder of **symlinks** — one per shared `.js`/`.html`/icon file back to the real file in `extension/`, plus `manifest.json` symlinked to `manifest.firefox.json` — so editing any shared file updates both browsers' loaded copies live with zero build step or bundler. `dist/firefox/manifest.json` is what gets pointed at Firefox's `about:debugging` → Load Temporary Add-on (or `web-ext run`) for live testing. Default `zip` behavior dereferences symlinks into real file content, so packaging a release zip for AMO submission from `dist/firefox/` needs no special flags. Remaining unknowns: live-test the DOM logic (row injection, focus/blur simulation, MAIN-world `confirm()` override) actually against Firefox's renderer — nothing here can be verified by code review alone; then AMO submission (self-distribution signing is fast/automated if not doing a public listing yet). Aside noticed during the audit, unrelated to Firefox: `content-lego-spy.js` isn't referenced in `manifest.json`'s `content_scripts` nor injected anywhere found in `content-lego.js` — looks like dead code, worth checking separately.
