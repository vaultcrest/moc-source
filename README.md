...
- [x] **PAB stock-status volatility tracking (2026-07-19, backend-only)**: `lego_element_stock_monthly`/`lego_element_stock_state` (see Database section above) track how often each element flips in/out of PAB stock and whether it's currently in stock, per region (na/eu/kr). Fed incrementally by `scripts/scrape_pab.py`'s `record_stock_events()`, hooked into the two write paths that touch `in_stock` at representative-locale granularity — sibling-locale propagation is deliberately not hooked to avoid double-counting the same real-world transition. No API endpoint or extension surfacing yet (see What's Next #12)
